...
We need to extract information on vassdragomrade for each observation (they are per smaller unit - so called regine), so that each observation also have a production area assigned to it.

Information on what is regine:
https://www.nve.no/kart/kartdata/vassdragsdata/nedborfelt-regine/
...
nonmatch
```
Examples of vassdrags that had no match in runoff table  (means  they do not intersect with aquaculture production areas):
Dalelven,
Kemijoki,
Torneelven,
Umeelven

Seems like it's all the rivers that cross the border between Norway and Sweden or Norway and Finland.
...
```{r}
write.csv(runoff_prep6, "./data/output/n_p_runoff_per_vassdragsomr_version2.csv")
```

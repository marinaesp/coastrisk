...
We have 13 years of data, and it would be difficult to plot them all. I will plot only the first and the last year of data series - 2005 and 2018, to identify temporal changes.
...
## Map aquaculture production for 2005
...
breaks3 <-seq(0,250000,by = 50000)
labels3 <- imap_chr(breaks2, function(., id){
  return(paste0(formatC(breaks2[id], format="f", big.mark="," , digits = 0),
                             "",
                            " – ",
                             formatC(breaks2[id + 1], format="f", big.mark="," , digits = 0),
                             ""))
})

labels3 <- labels3[1:length(labels3) - 1] #5 intervals in total
```
...
base_map <- ggplot() +
  ...
  scale_fill_manual(
    values = cols,
    name = "Aquaculture yield, 1000 tons",
    guide = guide_legend(

...
I need to do the same manipulations wiht municipalities' numbers as for production data to be able to merge standing biomass and production areas tables
```{r}
common <-aqua_prep5 |> 
  select(-c(fylke_kode,
            gjennomgatt,
...
Merging stnding biomass table with a production areas table. See chunk 37 for details.
...
Next, i will just select the variables of interest of each table and save the tables
...
```{r}
write.csv(fish_mort, "data/farmed_fish_mortality_per_mcp_and_prodregion.csv", row.names = F)
```

...
For how many rows of data there is NA in biomass change? 196, these are observations from 2005, when no previous year biomass data is available and some cases, where a municipality did not have production in the previous year. 
```{r}
sum(is.na(aqua_prep6$biomchange_salmon))
```

For these 196 cases, I have to calculate biomass change differently. Since I cannot subtract December from the previous year December,I look at the values of the given year only, and take the difference of the latest month and the earliest month with data (like if production started in February and continued through December of the give year).
...
NB!! Municipalities' numbers in this file and in the aquaculture production table are both per 2019 (not including changes of 2020). 
```{r}
...
Then I also correct the municipality number - where it has only 3 digits, should be  zero in front. Importantly, the table with municipalities and their production regions use municipalities numbers as they were before 2020. But for Trondelag, the municipalities numbers are as they were after 2018. However, the aquaculture production table uses a mix of both old and new numbers (as FISKEDIR explained this is a mistake, there is a confusion with municiaplities's numbers and they cannot fix that).
...
```{r}
View(filter(aqua_per_prodreg, is.na(prod_area_id)))
```
...
FISKEDIR explained they cannot help solve this confusion with municipalities' numbers, so just have to assume that the "knew" municipality, is one of the older ones.
...
aqua_per_prodreg_prep2 <- aqua_final %>% 
  left_join(convert_mcp_prep2[,c("municip_number_old",
                                 "municip_number_new")]
,
...
Molde 1502 and Tingvoll 1560 have matches in produciton areas, but the 0438 and 0436 municipalities are not there, neither under new nor under old numbers.
Do merging again but replaceing 1544 municipality's number and 1556 with 1502 and 1560, respectively.
```{r}
...
aqua_per_prodreg_final <-
  ...
  group_by(year, county, prod_area_name,prod_area_id ) %>% 

...
I need to clean the production areas table, to make areas names similar to other such tables in the project 
...
There are mismatches in the tables because Oslo should be 3 in the anlegg table, Sor and Nord-Trondelag both should be 
50, since they have merged in 2018. Also, other counties have a leading zero in their number in anlegg table, I need to remove it before I merge anlegg table with production areas.
...
anlegg_per_pa <- anlegg_prep2 |> 
  ...
  select(year, county, county_number, number_of_stations, prod_area_id, prod_area_name)
...
anlegg_per_pa2 <- anlegg_per_pa |> 
  ...
  summarize(number_of_stations_pa = sum(number_of_stations, na.rm=T)) |> 
  mutate(prod_area_name2 = str_c(prod_area_id, prod_area_name, sep = ": ")) |> 
  select(year, prod_area_id, prod_area_name, prod_area_name2, number_of_stations_pa)
...
organic_per_pa <- organic_prep2 |> 
  left_join(mcp_pa_prep, by = "county_number") |> 
  select(year, county, county_number, method, organic_discharge_tonn, prod_area_id, prod_area_name)
```
...
np_per_pa <- np_prep2 |> 
  left_join(mcp_pa_prep, by = "county_number") |> 
select(year, county, county_number, P_ton, N_ton, prod_area_id, prod_area_name)
```
